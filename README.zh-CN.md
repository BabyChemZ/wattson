...
- **它不是通用的异常检测器。** 一次判定需要至少两项独立证据，单凭 CPU 变高
  永远不够——那正是编译、导出、建索引每天都在做的事。它能可靠抓到的是这样一
  类问题：长期运行、有可辨识 I/O 特征的程序，突然不再干活却仍占满 CPU。代理
  卡死时吞吐归零，网络塌缩加 CPU 偏差，轻松过线；而一个纯计算的后台任务没有
  可塌缩的特征，实测只得 0.35，阈值是 0.5。同样够不到的还有：内存泄漏（判定
  里根本没有内存这一项）、单进程 GPU 失控（macOS 不提供该数据）、流量暴涨而
  非停滞、以及没有突变的长期缓慢劣化，还有本身状态就多到藏得住问题的程序——
  浏览器和 IDE 都在此列。阈值可以调低，但每一次误报都在教人忽略下一次，没人
  相信的看门狗还不如没有。
- **持续够久的问题会变成基线本身。** 行为是从程序实际做过的事学来的，所以一个长期卡死的程序，会把「卡死」学成它的正常状态之一。占比不足 15% 的聚类不被承认为常态，90 天的每日历史也会稀释掉某个糟糕的下午——但一个从未被观察到正常运行过的程序，无法用「正常运行」去衡量。
